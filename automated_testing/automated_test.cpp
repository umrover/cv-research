                ...
                switch (count) {
                    case 0:
                     //   t.hasTennisBall = boost::lexical_cast<bool>(fileList[i].substr(0, position));
                        t.hasTennisBall = (fileList[i].substr(0, position) == "1");
                        break;
                    case 1:
...
int main(int argc, char * argv[]) {
    ...
    if (d) {
        ...
        for (unsigned i = 0; i < testCases.size(); ++i) {
            vector<Point2f> centers = findTennisBall(testCases[i].img);
            // if it detects more than one tennis ball
            if (centers.size() > 1) {
        ...
        for (unsigned i = 0; i < testCases.size(); ++i) {
            cout << "Test " << i + 1 << ": " << testCases[i].hasTennisBall << "\n";
        }
